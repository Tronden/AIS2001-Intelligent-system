turtles-own [position-x position-y packet-loss noise]
...
;; Calculate the average position of turtles
to initial-average
  let mx mean [xcor] of turtles
  let my mean [ycor] of turtles
  print (list mx my)
  ask patch mx my [set pcolor red]
end
...
to go
   ask turtles [
    ...
    ask out-link-neighbors [
      ...
      set consensus-x consensus-x + ratio * position-x
      set consensus-y consensus-y + ratio * position-y
    ...
    if r < packet-loss
    [
      let noise-x (random-float 1 - 0.5) * (2 * noise) ; introduce noise
      let noise-y (random-float 1 - 0.5) * (2 * noise) ; introduce noise
      set position-x position-x + noise-x
      set position-y position-y + noise-y
    ]
    if r >= packet-loss
    [
